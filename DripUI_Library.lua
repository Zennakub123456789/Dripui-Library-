...
-- Based on MacUI, restyled to match DRIP CLIENT design
local DripUI = {}
...
-- DRIP Mobile Theme Colors
local DripTheme = {
    Background = Color3.fromRGB(32, 32, 38),
    TitleBar = Color3.fromRGB(38, 38, 44),
    Content = Color3.fromRGB(32, 32, 38),
    Accent = Color3.fromRGB(235, 30, 200), -- Pink/Magenta line
    AccentSecondary = Color3.fromRGB(52, 199, 89), -- Green for toggles ON
    ...
    ElementBg = Color3.fromRGB(42, 42, 48),
...
function DripUI:Window(config)
    ...
    self.IsExpanded = true -- true = full UI, false = collapsed (title only)
    ...
    -- Main Frame
    local windowWidth = config.Width or 420
    local windowHeight = config.Height or 400
    local titleBarHeight = 40
    local tabBarHeight = 40
    local accentLineHeight = 3
    ...
    -- Title Bar Button (for expand/collapse)
    ...
    function self:SelectTab(index)
        ...
        for i, tab in ipairs(self.Tabs) do
            ...
            if tab.Button then
                if i == index then
                    tween(tab.Button, 0.2, { BackgroundColor3 = DripTheme.TabActive, BackgroundTransparency = 0 })
                else
                    tween(tab.Button, 0.2, { BackgroundTransparency = 1 })
                end
            end
        end
    end
    ...
    function self:Tab(name)
        ...
        Button.MouseEnter:Connect(function()
            if self.SelectedTab ~= tabIndex then
                tween(Button, 0.15, { BackgroundColor3 = DripTheme.TabHover, BackgroundTransparency = 0 })
            end
        end)
        ...
        function tab:Toggle(cfg)
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                tween(toggleKnob, 0.2, { Position = toggleState and UDim2.new(1, -22, 0.5, -10) or UDim2.new(0, 2, 0.5, -10) })
        ...
        function tab:Dropdown(cfg)
            ...
            local optionsFrame = create("Frame", {
                ...
                Position = UDim2.new(0.4, 10, 0, 38),
                BackgroundColor3 = DripTheme.DropdownBg,
                BorderSizePixel = 0,
                ClipsDescendants = true,
                Visible = false,
            ...
            create("UIPadding", { Parent = optionsList, PaddingTop = UDim.new(0, 4), PaddingBottom = UDim.new(0, 4) })
            ...
            if cfg.Options then
                for _, option in ipairs(cfg.Options) do
                    ...
                    optionBtn.MouseEnter:Connect(function()
                        tween(optionBtn, 0.1, { BackgroundTransparency = 0, BackgroundColor3 = DripTheme.DropdownItemHover })
                    end)
                    ...
                    optionBtn.MouseButton1Click:Connect(function()
                        ...
                        tween(optionsFrame, 0.2, { Size = UDim2.new(0.55, 0, 0, 0) })
                        arrow.Text = "▼"
            ...
            dropBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    local height = math.min(optionCount * 32 + 8, 150)
                    tween(optionsFrame, 0.2, { Size = UDim2.new(0.55, 0, 0, height) })
                    arrow.Text = "▲"
                else
                    tween(optionsFrame, 0.2, { Size = UDim2.new(0.55, 0, 0, 0) })
        ...
        function tab:Button(cfg)
            ...
            btn.MouseEnter:Connect(function()
                tween(btn, 0.15, { BackgroundColor3 = Color3.new(DripTheme.Accent.R * 0.85, DripTheme.Accent.G * 0.85, DripTheme.Accent.B * 0.85) })
            end)
        ...
        function tab:Slider(cfg)
            ...
            local sliderAPI = {
                Set = function(_, newValue)
                    ...
                    tween(sliderKnob, 0.2, { Position = UDim2.new(pos, -8, 0.5, -8) })
                    SavedData[cfg.Title] = value
                    SaveConfigData()
                    if cfg.Callback then cfg.Callback(value) end
                end,
